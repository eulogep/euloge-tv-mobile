...
type RailSection = { title: string; detail: string; channels: MjtvChannel[] };
...
export default function HomeScreen() {
  ...
  const featured = channels.find((channel) => channel.canOpen) ?? null;
  const railSections = useMemo<RailSection[]>(() => {
    const recommended = channels
      .filter((channel) => channel.canOpen)
      .slice(0, 12);
    ...
    return [
      {
        title: "Pour vous",
        detail: "Sélection MJTV selon la qualité des sources",
        channels: recommended,
      },
      {
        title: "Disponibles maintenant",
        detail: "Sources confirmées ou dégradées",
        channels: confirmed,
      },
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <>
            ...
            {catalogError ? (
              <Pressable onPress={refreshCatalog} style={styles.networkNotice}>
                <MaterialIcons name="cloud-off" size={17} color="#F6C85F" />
                <View style={styles.networkCopy}>
                  <Text style={styles.networkNoticeTitle}>
                    Catalogue temporairement indisponible
                  </Text>
                  <Text style={styles.networkNoticeText}>
                    Touchez pour réessayer. Aucun contenu de démonstration n’est
                    affiché.
                  </Text>
                </View>
              </Pressable>
            ) : null}
            {!catalogLoading && !catalogError && !featured ? (
              <View style={styles.empty}>
                <MaterialIcons name="tv-off" size={34} color="#747386" />
                <Text style={styles.emptyTitle}>Aucune chaîne disponible</Text>
                <Text style={styles.emptyText}>
                  Le catalogue MJTV ne contient actuellement aucune chaîne
                  ouvrable.
                </Text>
              </View>
            ) : null}
              ...
              <>
                ...
                >
                  <View style={styles.heroArtwork}>
                    <ChannelArtwork channel={featured} />
                  </View>
                  <View style={styles.heroContent}>
                    <View style={styles.heroBadgeRow}>
                      <StatusBadge status={featured.status} />
                      <Text style={styles.heroFlag}>{featured.country}</Text>
                    </View>
                    ...
                    <View style={styles.heroActions}>
                      ...
                      <Pressable
                        accessibilityRole="button"
                        accessibilityLabel="Ajouter la chaîne à Ma liste"
                        onPress={(event) => {
        ...
        ListFooterComponent={
          <View style={styles.footer}>
            <Text style={styles.footerText}>
              MJTV référence des sources publiques. Disponibilité non garantie.
            </Text>
          </View>
        }
...
const styles = StyleSheet.create({
  ...
  hero: {
    ...
    borderRadius: 24,
    borderWidth: 1,
    borderColor: "rgba(162,123,255,0.42)",
  ...
  networkNotice: {
    ...
    alignItems: "center",
    gap: 10,
    backgroundColor: "rgba(246,200,95,0.1)",
    borderWidth: 1,
    borderColor: "rgba(246,200,95,0.26)",
  },
  networkCopy: { flex: 1 },
  networkNoticeTitle: { color: "#F6C85F", fontSize: 12, fontWeight: "800" },
  networkNoticeText: {
    color: "#C9B979",
    marginTop: 3,
    fontSize: 10,
  ...
  empty: { alignItems: "center", paddingHorizontal: 40, paddingVertical: 54 },
  emptyTitle: {
    color: "#F5F7FF",
    marginTop: 14,
    fontSize: 17,

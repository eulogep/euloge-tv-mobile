...
export default function LiveScreen() {
  const { channels, catalogLoading } = useMjtv();
  ...
  return (
    <ScreenContainer style={styles.screen}>
      <FlatList
        ...
        ListHeaderComponent={<><MjtvHeader title="Live" />{catalogLoading && <LoadingSkeleton rows={2} />}<View style={styles.summary}><View style={styles.liveDot} /><View><Text style={styles.summaryTitle}>{available.length} chaîne{available.length > 1 ? "s" : ""} en direct</Text><Text style={styles.summaryText}>États et disponibilités mis à jour à l’ouverture.</Text></View></View></>}
